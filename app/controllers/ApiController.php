        ...
        if ($requestException->CorsHeader()) {
            ...
            if($security_check->checkBearerToken() || $security_check->checkBearerToken() ==true){
                ...
                if($authClass->authCheck()){
                    $security = new SecurityFilterChain();
                    if($security->isValidToken()){ 
                        if (RequestHandler::isRequestMethod('GET')) {
                            $jwt  = new JwtService();
                            if (isset($_GET['action']) && isset($_GET['v1'])) {
                                $action = $_GET['action'];
                                if ($action=='get_journal_year_list'){
                                    if (isset($_GET['query']) && !empty($_GET['query']) && $_GET['query']=='publish_year_list' && !empty($_GET['journalId']) && is_numeric($_GET['journalId'])) {
                                        $journalId = strip_tags(filter_var($_GET['journalId'], FILTER_SANITIZE_STRING));
                                        $response['data']= $this->repository->get_journal_details_by_id($journalId);
                                        echo json_encode($response);
                                        http_response_code(200);
                                    }else{
                                        $response = array("status"=>"error","inc"=>false, "data"=>'publish year', "message"=>'Missing paramenters on fetching data for publish year.');
                                        echo json_encode($response);
